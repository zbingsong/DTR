...
def bootstrap_confidence_interval(metric_fn, preds, labels, n_bootstraps=1000, ci=0.95):
    ...
    for i in range(n_bootstraps):
        ...
        if len(np.unique(labels[indices])) < 2:
            continue
        score = metric_fn(labels[indices], preds[indices], average='weighted')
...
def main():
    ...
    train_dataset = ExtractedDataset(args.pth_file, 'train')
    val_dataset = ExtractedDataset(args.pth_file, 'val')
        ...
        with torch.no_grad():
            for he, ihc, labels in val_pbar:
                ...
                if args.mode == 'HE':
                    ...
                    raise ValueError("Invalid mode. Choose from ['HE', 'HE&IHC']")
